...
class Array
  def bubble_sort!(&prc)
    ...
    while !sorted
      ...
      (0...self.length).each_with_index do |i|
  ...
  def bubble_sort(&prc)
    ...
    while !sorted
      ...
      (0...arr_copy.length).each do |i|

        if prc.call(arr_copy[i], arr_copy[i + 1]) == 1
...
def subwords(word, dictionary)
  output = substrings(word).uniq
  output.select {|ele| dictionary.include?(ele)}
end
